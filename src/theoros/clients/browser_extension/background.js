// background.js — Theoros browser extension background script (Firefox MV3 event page)
//
...
//   dwell_timer:           number|null — setTimeout handle
//   unfocused_reset_timer: number|null — setTimeout handle
...
function clearTabState(tabId) {
  ...
  cancelUnfocusedResetTimer(tabId);
...
function onTabUnfocused(tabId) {
  ...
  // Start 10-minute unfocused clock.  If it elapses, the tab gets a
  // fresh session_id — any subsequent capture is a new session.
  state.unfocused_reset_timer = setTimeout(() => {
    state.unfocused_reset_timer = null;
    state.tab_session_id = crypto.randomUUID();
    console.log(
      `[theoros] tab ${tabId}: 10min unfocused, new session_id ${state.tab_session_id}`
    );
    persistTabSession(tabId);
  }, UNFOCUSED_RESET_MS);
  console.log(`[theoros] tab ${tabId}: unfocused reset timer started (10min)`);
}
...
function cancelUnfocusedResetTimer(tabId) {
  const state = tabStates.get(tabId);
  if (!state?.unfocused_reset_timer) return;
  clearTimeout(state.unfocused_reset_timer);
  state.unfocused_reset_timer = null;
  console.log(`[theoros] tab ${tabId}: unfocused reset timer cancelled`);
}
...
function onPause() {
  ...
  // Cancel all timers across all tracked tabs.
  for (const [, state] of tabStates) {
    if (state.dwell_timer) {
      clearTimeout(state.dwell_timer);
      state.dwell_timer = null;
    }
    if (state.unfocused_reset_timer) {
      clearTimeout(state.unfocused_reset_timer);
      state.unfocused_reset_timer = null;
    }
  }
...
chrome.storage.local.get("isPaused").then((result) => {
  ...
  loadPersistedTabSessions().then((stored) => {
    console.log(
...
// hydrate the cache from the server. Both events fire when the event
// page genuinely (re)loads after a lifecycle transition: onInstalled on
// install / update / browser update / developer reload, onStartup on
// browser cold start. We deliberately don't run this at top-level eval
// — on MV3 event pages that races on fresh install (script can wake
// before the service is reachable, with no retry) and also fires on
// unrelated event-driven wake-ups, which would beat on /exclusions/urls
// for no reason. Migration is gated by exclusions_migrated_v1 so the
// extra call from onStartup is a no-op once it's run once.
